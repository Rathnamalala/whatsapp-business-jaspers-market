...
// Map of callId → { dgConnection, sdpAnswer, isProcessing }
 ...
 *   5. Audio loop: RTCAudioSink → Deepgram → Claude → ElevenLabs PCM → RTCAudioSource
 */
async function startCallSession(callId, sdpOffer, senderPhoneNumberId, callerNumber) {
  ...
  dgConnection.on("message", async (data) => {
    ...
    try {
      ...
      // PCM at 44100 Hz — RTCAudioSource resamples to 48kHz internally
  ...
  dgConnection.on("error", (err) => {
    console.error(`[call-session] Deepgram error for call ${callId}:`, err);
  });
  ...
  const { sdpAnswer } = await WebRTC.handleIncomingCall(
    ...
    sdpOffer,
    (pcmBuf) => {
      // Send raw PCM to Deepgram once the WebSocket is open
      try {
  ...
  // ── 3. Pre-accept (establishes media path before audio clipping) ───────────
  try {
